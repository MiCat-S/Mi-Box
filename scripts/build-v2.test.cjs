...
test('service template preserves direct startup, group cleanup, and journal logging', () => {
  ...
  assert.ok(directives.includes('ExecStart=@NODE@ @RUNTIME@ --serve'));
  assert.ok(directives.includes('WorkingDirectory=@ROOT@'));
  ...
  assert.match(service, /Generate after foreground account verification/);
