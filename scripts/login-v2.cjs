...
async function login() {
  ...
  try {
    await fs.lstat('config.json');
    throw new Error('config.json exists; preserve it or move it to a private backup before login');
  }
  ...
  try {
    ...
    await client.start({
      phoneNumber: () => {
        stage = 'phone-number';
        return ask('Phone number (+country code): ');
      },
      phoneCode: () => {
        stage = 'verification-code';
        return ask('Telegram login code: ', true);
      },
      password: () => {
        stage = 'two-step-password';
        return ask('Two-step verification password: ', true, false);
      },
