...
Core 维护运行时加载的 19 个内置模块；leech、re、sure 的实现与测试统一由扩展仓库维护。扩展仍为 115 个入口、165 个命令键和 16 个监听器，插件身份、配置位置和按需安装方式保持一致。ai/gt 从同一份扩展源码默认打包，exec 扩展仅声明历史兼容身份。

此前分批验收记录反映 0.5.0 迁移时的源码范围；当前归属以本节和两仓开发说明为准。

0.5.1 验证（Node v24.20.0，隔离副本）：

- `npm run test:v2`：1772 total / 1771 pass / 0 fail / 1 skip，exit 0；跳过项仍为 macOS 不适用的 Linux 内核锁用例。
- `npm run test:plugins:v2`：115/115 构建、加载、卸载与资源回收通过；165 个命令键归属唯一、16 个监听器保留。
- 跨仓库归属检查覆盖主动业务声明与被动兼容入口；扩展回归覆盖 sure 授权、大小写、名单、重载持久化，leech 真实 SQLite 统计，以及 re 转发范围、次数和失败行为。
- Core 11 个文件（含三项删除）、Plugins 4 个文件与验证副本逐字节核对一致，验收记录在验证后补齐。两仓差异空白检查通过，三处版本均为 0.5.1。
- 验证目录：`/var/folders/f6/cw7nkc6x6pz_rcrf90ky94gh0000gn/T/mibox-review-ownership-051-fxl1kgw2`，日志为 `verification.log`、`plugin-verification.log`。
